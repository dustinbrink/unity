...
public static class ActionMaster {
	...
	public static Action NextAction ( List<int> rollsOrig ) {
		...
		} else if ( rolls.Count >= 19 && roll == 10 ) { // Handle last-frame special cases
			nextAction = Action.Reset;
		} else if ( rolls.Count == 20 ) {
			if ( rolls[ 18 ] == 10 && rolls[ 19 ] < 10 ) { // Roll 21 awarded
				nextAction = Action.Tidy;
			} else if ( rolls[ 18 ] + rolls[ 19 ] == 10 ) { // Spare, extra roll
				nextAction = Action.Reset;
			} else {
				nextAction = Action.EndGame;
			}
		} else if ( rolls.Count % 2 != 0 && roll != 10 ) { // First bowl of frame & not strike
			nextAction = Action.Tidy;
		}
	...
	public static int CountFrames ( List<int> rolls ) {
		int frames = 0;
